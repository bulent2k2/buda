...
def sole_path_arg(cmd_line):
    ...
    `do_command` splits on whitespace, so `args[0]` is a path only up to its
    first space: `source my dir/flow.buda` looked for `my`, and the error named
    a file the author never wrote.  It bit the FLOW's own path hardest, since
    that is the one path a user does not choose — `bin/buda "my dir/x.buda"`
    reaches the engine as `source my dir/x.buda`, and `btcl -i` sends the same
    line — so a checkout under `~/My Designs/` could not be run at all.
    ...
    """
    rest = _strip_inline_comment(cmd_line).strip().split(None, 1)
    if len(rest) < 2:
        return ""
    path = rest[1].strip()
