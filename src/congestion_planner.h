...
private:
    ...
    void rebuild_cuts_();
    // ── Victim-ladder footprint pruning (replan_bundle_ripup, B1 PR2) ──────
    ...
    // width eff+pitch, ±1 rounding pad).  `seg` tags the owning segment so the
    // decoupling test can tell same-segment rects (different layers) from a
    // genuine cross-segment overlap.  Capacities/patterns/keepouts are static
    // during a trial, so usage is the only live input to feasibility.
    struct BandRect {
        int    seg;          // owning segment index (for cand_is_decoupled_)
        int    layer_id;
    ...
    // kBalance-free — the candidate's feasibility is a pure function of usage
    // on its own footprint bands.  This is exactly the property that makes the
    // "disjoint from the rip ⟹ feasibility unchanged" prune SOUND; an
    // overlay-COUPLED candidate can flip feasible via a kBalance-driven layer
    // change on unchanged usage (the #629 unsoundness Codex caught), so those
    // are never pruned.  Conservative: two segment-rects on the same
    // (layer, direction) whose along-spans AND perp-windows both overlap are
    // treated as coupling even if no actual cut sits in the overlap.
    bool cand_is_decoupled_(const std::vector<BandRect>& rects) const;
    ...
    // footprint pruning, whose soundness requires the mask to be a SUPERSET of
    // the feasible set — see cand_is_decoupled_ and the ladder comment.
