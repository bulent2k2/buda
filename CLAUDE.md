...
| `generate_hier_topologies [center_mode] [double_detour]` | 2 | Generate candidates for all HBundles (3-case: cell-local / cross-level / cross-block). Both `generate_[hier_]topologies` **persist** all candidate topologies into the BDB `topology`/`topology_segment` tables (before `run_planner`) when a BDB is open; see [BDB Reference](docs/BDB_REFERENCE.md) |
