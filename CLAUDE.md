...
| `check_design [all]` (alias `check_connectivity`) | verify | Run the design audit at the current stage (topo / NUTS / detailed-NUTS): connectivity opens, layer directions, keepout crossings, unplaced bits. `all` checks every candidate topology; when run pre-planning (no topology selected yet) the topo stage auto-promotes to all-candidates mode. At the NUTS/DNUTS stages it also prints the **supply-doomed seat** census (issue #536 diagnostic): every placed segment whose assigned layer's REAL signal-track supply in the placed window is below its member bits by the exact DNUTS admission arithmetic — a static width-infeasibility where every bit is guaranteed to strand (the mix2 b61 shape: 16 bits committed on a 12-track M6 window by a measured healer move), covering the TOP layers the dead-span escalation family cannot reach and separating these from DNUTS's dynamic reservation-conflict warnings; locked bottom-up copies are excluded (their bits arrive via the template-copy path, which never runs per-instance admission). Report-only — it never filters or aborts (generation's coverage gate is what drops uncovered candidates) |
