...
| `set_drop_dangling [on\|off]` | 2 setup | Opt-in dangling/unclamped-segment candidate drop (default off — bit-identical without it): at the end of every generation command (not `generate_more_topologies`), drop a candidate that has a DANGLING segment (a ConnSeg with a single non-block connection — a wire whose other end connects to nothing) or an UNBOUNDED slide window (the `±2^30` no-clamp sentinel) — the OOB detour / MST-relay geometry that passes the coverage gate (every block tapped, one connected graph) but renders as a dangling overshoot (e.g. b44's `TRUNK_*_OOB(+MST)` candidates). Never drops a USER or the pinned candidate, and never strands a bundle (keeps the pool if every candidate is flagged). Prints a per-candidate `[TopoDangling]` note; runs BEFORE sidecar restore + persistence (declare before generating; pins from an opted-in run) |
...
| `unpin_topology <hint\|*>` | 3 | Clear a bundle's pin (inverse of `select_topology`): drops `topology_pinned` AND any forced `pinned_seg_layers` (from `edit_commit pin` after `edit_set_layer` — the planner honors those for any candidate, so a re-chosen topology would otherwise keep the stale layers) so the next `run_planner` may re-choose, leaving the current selection in place; `*` clears every pin |
