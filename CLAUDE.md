...
- **BDB bus-width editor** — `tools/bdb_edit_bus.py`: resize a bus's bit count (prune 64→4, grow 2→16 by cloning a template bit's pins) or delete a bus, editing the netlist directly in a `.bdb`/`.bdb.sql` across every net-referencing table (net/pin/net_props/bundle_net/net_segment/net_via). `--list` enumerates buses; `--dry-run` previews; `--clear-routing` drops stale route tables. Re-run the flow afterward.
