...
| `run_detailed_nuts [lo_hi\|hi_lo]` | 9 | Snap bit-wires to concrete tracks; emits per-bit `net_vias` (the symbolic bus-vias fanned out per bit, drawn under `[Vias/Conns]` in detailed viz). Track choice is **span-clear-first** (#523 spreader outcome): when the midpoint-fallback pool engages, tracks clear across the whole abstract span (bits there can never be keepout-culled) are claimed before midpoint-only ones. After the solve, the **measured cull heal** (`_final_cull_heal`, default on) escalates cull-doomed LOW segments (measured stranding + span-clear pool < member bits — ripup's refold tier, for the healerless flows that never reach it) under a COMPONENTWISE accept (opens strictly down, overlaps never up; batch bisected on rejection), scoped out of bottom-up sessions (`hier.locked`, the width-gate boundary — a locked flow's downstream healers re-roll from the changed start state); measured bigHalf no-rr 102→0/0, chip3_topdown unplaced −13%, 32/34 corpus flows unchanged. **Persists** bit-wires + per-bit vias into the BDB `net_segment`/`net_via` tables (FK to `bundle`, net identity via `net_id`) and rewrites `route_snapshot` (stage `detailed_nuts`) when a BDB is open; see [BDB Reference](docs/BDB_REFERENCE.md) |
