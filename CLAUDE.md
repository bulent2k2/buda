...
| `align_bottom_up [max_shift <um>]` | Nudge every `set_bottom_up` cell's instances onto a common track phase with **minimal total movement** (per axis: coordinate mod the LCM of that direction's layer pitches; the target phase is the circular L1 median of the instances' phases, so the majority usually stands still). Subtree-aware (`translate_comp`), so congruence is preserved; region overrides are absolute-keyed and not fixable by translation. Run after `def_track_pattern` + `set_bottom_up`, BEFORE `derive_busterms`/`add_blocks_from_bdb`; `max_shift` skips any larger nudge with a WARNING |
