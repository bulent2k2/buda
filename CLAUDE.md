...
| `unpin_topology <hint\|*>` | 3 | Clear a bundle's pin (inverse of `select_topology`): drops `topology_pinned` so the next `run_planner` may re-choose, leaving the current selection in place; `*` clears every pin |
