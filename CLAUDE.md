...
- **GDSII import + export** — round-trip a GDSII layout against BDB.
  ...
  - *Import:* read GDSII structures back into BDB component/cell rows — `BOUNDARY`/`BOX` shapes become cell or blockage geometry, `SREF`/`AREF` placements rebuild the component hierarchy, layer→`(layer, datatype)` mapping is inverted. GDS carries **geometry only, no logical netlist**, so a GDS import recovers placement/shapes but not net connectivity (pair it with Verilog for nets, as with DEF today).
