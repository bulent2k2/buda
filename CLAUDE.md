...
| `run_bundler <STRICT\|CONVERGENT\|BIDIRECTIONAL>` | 1 | Group nets into buses (CONVERGENT = fan-in by shared receiver; BIDIRECTIONAL = direction-agnostic, sorted set of all endpoint instances, so A→B bundles with B→A and cyclic a→b,c/b→c,a/c→b,a. A CONVERGENT bundle spanning multiple driver *blocks* routes as a **fan-in tree**: generation derives endpoints from ALL the bundle's nets and roots the multicast/MST shapes at the shared sink with every driver as a leaf, so every driver block is physically attached — and `check_design`'s net-driver fidelity check (`NET_DRIVER_OPEN`) flags any topology whose block contract drops an endpoint. BIDIRECTIONAL connects the same blocks so it's sound by construction. See docs/internal/convergent_bundling.md) |
