...
| `edit_topology <bundle_id> [<cand#>\|new]` + `edit_add_trunk <H\|V> <perp> [<lo> <hi>] [layer <id>]` / `edit_add_stub <block> <seg#>` / `edit_set_span <seg#> <lo> <hi>` / `edit_set_layer <seg#> <layer_id>` / `edit_connect <i> <j>` / `edit_disconnect <i> <j> <retract_to>` / `edit_remove_segment <seg#>` / `edit_status` / `edit_commit [pin]` / `edit_abort` | 2 | **TopoEdit session**: open a working copy of a candidate (or an empty topology), apply transactional edits — pick an axis + Hanan line and add a (default full-span) trunk, stub blocks to it, override spans, connect/disconnect perpendicular segments — each printing a verdict (check_topo violations + zero-slide pinch + wire-graph components); `edit_commit` appends the result to the bundle's pool as a `USER` candidate (uid-deduped; `pin` selects it), `edit_abort` discards. Slide overrides ride the existing `plan.seg_slide_lo/hi` NUTS hatch |
