...
| `generate_topologies [center_mode] [double_detour] [multi_trunk]` | 2 | Generate candidates for all bundles (src/dst auto-derived from netlist). `multi_trunk` (opt-in) adds two-level **BITRUNK_HVH/VHV** trees for high-fan-out datapath nets: a root spine feeds perpendicular branch trunks, each tapping a cluster of aligned blocks (a column/row becomes a multi-tap pass-through trunk), emitted in both orientations |
