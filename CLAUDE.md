...
| `export_gds <file.gds> [outline <l>] [labels <l>\|off] [via_size <um>]` | Export the BDB as deterministic GDSII (reverse of `import_gds`, from the **persisted** tables): cells → structures + SREFs, `net_segment` wires (`bus_segment` fallback) on mapped pairs, vias as squares, one net-name TEXT per pin — re-importable with the same map (tested round-trip) |
