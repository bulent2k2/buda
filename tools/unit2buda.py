...
Two capture modes, auto-selected by what the test actually does:
  ...
  * **CLI/BDB flow** — the test drives a ``buda.BudaSession`` via ``do_command``
    (e.g. the hier bundler / planner / NUTS flow): the command stream is recorded
    and emitted, then the remaining pipeline stages are appended so the script is
    a full, visualizable run — ``generate_topologies`` + ``run_planner`` (flat) or
    ``generate_hier_topologies`` + ``run_planner hier`` (hier), then ``run_nuts``,
    ``run_detailed_nuts``, ``visualize`` (stages the test already ran are not
    duplicated).
...
def _is_hier_flow(cmds):
    """True if the captured command stream is a hierarchy-aware (BDB) flow."""
    firsts = [c.split()[0] for c in cmds if c.split()]
    if "run_hier_bundler" in firsts or "generate_hier_topologies" in firsts:
        return True
    if "run_bundler" in firsts or "generate_topologies" in firsts:
        return False
    return "open_bdb" in firsts   # BDB opened but not bundled yet → hier flow


def emit_flow_script(cmds, test_spec, notes):
    """Emit the recorded BudaSession.do_command stream, then append whatever
    downstream stages are missing so the script is a full, visualizable pipeline:

        <flat>  generate_topologies       | <hier> generate_hier_topologies
                run_planner               |        run_planner hier
        run_nuts / run_detailed_nuts / visualize   (both flows)

    A stage already present in the captured stream is not re-appended (present
    tokens are a pipeline prefix, so this just completes the run)."""
    out = [f"# Generated by tools/unit2buda.py from {test_spec}",
           "# CLI/BDB-flow case: the BudaSession.do_command sequence the test ran,",
           "# then the remaining pipeline stages appended to make it a full run."]
    for n in notes:
        out.append(f"#   {n}")
    out.append("")
    out.extend(cmds)

    hier = _is_hier_flow(cmds)
    tail = (["generate_hier_topologies", "run_planner hier"] if hier
            else ["generate_topologies", "run_planner"])
    tail += ["run_nuts", "run_detailed_nuts", "visualize"]
    present = {c.split()[0] for c in cmds if c.split()}
    appended = [t for t in tail if t.split()[0] not in present]
    if appended:
        out.append("")
        out.append(f"# --- pipeline completion appended by unit2buda "
                   f"({'hier' if hier else 'flat'} flow) ---")
        out.extend(appended)
    out.append("")
    return "\n".join(out)
...
def main():
    ...
    # is an implementation detail of the CLI commands — the do_command stream is
    # the real intent.  Direct-API topology tests never touch a session.
    if _CMD_CALLS:
        ...
        sys.exit(f"error: {func_name!r} neither drove a BudaSession nor called "
                 "TopologyGenerator.generate_candidates "
