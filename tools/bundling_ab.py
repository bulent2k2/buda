...
the CLI writes every run's detail) for anyone checking a row.  A flow with no bundler in its own text is refused — one
reached through `source` cannot be injected before — and so is a flow that
already sets `set_max_bundle_bits`, whose own cap (or a scoped rule, which
outranks the global one for its prefix) would make "one net per bundle"
untrue for part of the design while the table still said it.
...
from buda_script import strip_inline_comment  # noqa: E402
...
def unbundled_text(text):
    ...
    flow already declares a bundle-bit cap of its own."""
    lines = text.splitlines(keepends=True)
    words = [_command(ln) for ln in lines]
    if "set_max_bundle_bits" in words:
        i = words.index("set_max_bundle_bits")
        raise Refused(
            f"line {i + 1} already sets set_max_bundle_bits; its cap, or a "
            f"scoped rule that outranks the global one for its prefix, "
...
def summarize(report, stdout):
    """One arm's row: counts from the run's output, times and verdicts
    from its --report-json."""
    cmds = report.get("commands", [])
    bundles = _last_int(r"HierBundler: (\d+) hbundles", stdout)
    if bundles is None:
        bundles = _last_int(r"Bundler created (\d+) hbundles", stdout)
    cands = _last_int(r"(\d+) total candidates", stdout)
    if cands is None:
        per = re.findall(r"Generated (\d+) topologies for bundle", stdout)
        cands = sum(int(n) for n in per) if per else None
    ...
    return {
        ...
        "detailed_wl": _last_int(r"total detailed WL = (\d+)", stdout),
    }


def run_arm(flow, text, tag):
    """Run `text` as a sibling of `flow`; return (report, stdout)."""
...
def main(argv=None):
    ...
    text = Path(args.flow).read_text()
    try:
        variant = unbundled_text(text)
    except Refused as e:
    ...
    try:
        for tag, t in (("bundled", text), ("unbundled", variant)):
            ...
            report, stdout = run_arm(args.flow, t, tag)
            rows[tag] = summarize(report, stdout)
