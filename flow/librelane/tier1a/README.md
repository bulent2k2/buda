...
| `drc_locate.py <drc.klayout.lyrdb> <top.def> [<cell>.lef ...]` | the reader for a top run's KLayout DRC report (#896): maps every marker from TOP coordinates to the instance holding it and the CELL-LOCAL spot (placement and orientation inverted), groups equal spots -- five `m2.2` markers at `acc_cell` local (69.4, 0.0) are ONE defect -- and, with the macro LEFs, says per offending edge whose metal it is on: a pin/OBS rectangle the LEF claims, inside the macro on an obstructed layer with no LEF shape under it (the GDS metal the abstract does not cover; a router cannot have put a wire there), over the macro on an unobstructed layer (the top's routing), or outside the box (routing against the macro edge).  A reader, not a check: it decides which experiment to run first, not whether the violation is real |
