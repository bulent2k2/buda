...
| `pdn_connect.py <pdn.def> [<cell>.lef ...]` | the check to run AFTER the top's PDN stage: reads the DEF **pdngen wrote** and reports, per macro power pin, whether a via landed on it and -- where none did -- whether the pin had a same-net crossing on the other `add_pdn_connect` layer at all.  Nothing is modelled: every cut, halo and obstruction subtraction has already happened by the time that metal is in the file.  `--self-cross <lef>...` asks the LEF-only half (does a power pin cross its OWN net on the other connect layer -- a per-cell property that connects or floats a net on every phase, which no offset search can fix).  The post-mortem twin of `pdn_phase.py` |
