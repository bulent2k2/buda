...
| `pdn_phase.py <top config.json> [<cell>.lef ...]` | the check to run AFTER hardening and BEFORE the top: reads the hardened macros' VPWR/VGND pin rectangles **and their `OBS` blocks** and the top's PDN config and PREDICTS what pdngen builds, step by step as OpenROAD's `src/pdn` does it -- the straps, every CUT a pin or OBS makes in them (two spacings across, a touch included -- `Shape::cut`'s query is a closed box intersection -- and the halo plus two spacings along; no same-net spare, since the macro grid's own copy of a pin carries no net; #904), the vias the macro grid's `add_pdn_connect` makes at same-net cross-layer overlaps, and the TRIM that removes every fragment with fewer vias than the layer needs (one, trim or no trim: `Shape::writeToDb` refuses a via-less shape) -- then partitions what is left with `pdn_connect.py`'s own `net_components` and reports every TRIM (information), every STRANDED terminal (a pin none of whose rectangles is on a component holding a surviving fragment -- with `PDN_ENABLE_PINS` every surviving fragment is written as a pin shape, a PSM source), every FLOATING fragment (survives trim off the largest component: a pin shape of its own with pins on, reported, else the "unconnected shapes" `PSM-0069` counts), and the smallest whole-placement shift (or the equivalent PDN_VOFFSET/PDN_HOFFSET) predicted to clear it -- a HYPOTHESIS for `check_grid.tcl`, since its remedy measured wrong once (§11 item 12).  **ADVISORY** -- a prediction from the LEFs and the config, not the verdict, which is OpenROAD's own PSM at the end of the top run; it does not gate `harm.sh`.  Its older version counted every same-layer meeting as a defect, and acting on that FAIL by hand once turned a working plan into `PSM-0069` (§11 item 8; #895) |
...
| `corridor_notch_check.py <arm>` | **read-only**, after `guides.sh`: do BUDA's corridors ever overlap the metal `notch.sh` adds?  `guides.sh` plans against MAGIC's abstract and the top routes against the PATCHED one, so the two views differ exactly inside the macros (#913).  **Not inert**: on the N=4 H+B arm the guide carries **11,311** boxes overlapping notch metal (51.3 µm² of it, counting shared regions once).  An earlier reading said inert by 0.555 µm and was measuring the wrong file -- `buda_guides.json`, the abstract manifest, where corridors stop at the macro boundary; `read_guides` consumes `buda_bus.guide`, whose `terminal` strips reach INTO macros to touch pins, which is where the notch metal is.  Both arms still route clean (guides are advisory), so what this reports is a real disagreement the router absorbs, not a broken run.  It **REFUSES** rather than reporting a zero it did not measure: no notch piece placed, no corridor read, or no layer shared between them each stop it, and liveness is reported PER LAYER because a layer carrying corridors and no notch metal (met3 here) contributes a zero that is arithmetic, not evidence |
...
| `tap_census.py <placed.def>` | does every standard-cell ROW fragment that holds a cell also hold a well tap?  Read on the detailed-placement DEF, before the router is paid for: an untapped fragment is a floating n-well once fill pads it out, and a REAL cell placed there (a clock-delay buffer, typically) is the LVS fault the compact arm measured (978 errors from 764 such fragments -- edge cells narrower than the PE pitch leaving 26.7 um gaps that the 10 um macro halos cut to 6.7 um, too short for the 13 um tap pitch).  The macro halo (`harm.sh --halo`) shapes the fragments, `FP_TAPCELL_DIST` taps them; exit 1 names what either left out |
| `render_arms.py placement|routing --layers L,L|wire|utilization [--out PREFIX]` | the PICTURES and the per-layer numbers of the three arms at N = 8, from their signoff DEFs, all three at ONE scale, with a hierarchical arm FLATTENED (every hardened block's own final DEF placed back at its FIXED location): the standard-cell placement coloured by kind with block and PE-row outlines, the signal wires of chosen layers (NETS only, never the SPECIALNETS grid), wire per layer split top / blocks / flattened, and logic-cell area over die and core.  Pinned against LibreLane's own figures: top-level wire within 20 um of each arm's `route__wirelength`, F's logic + tap area within 0.02 % of `design__instance__area__stdcell` |
