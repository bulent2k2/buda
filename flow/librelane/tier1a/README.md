...
| `corridor_notch_check.py <arm>` | **read-only**, after `guides.sh`: do BUDA's corridors ever overlap the metal `notch.sh` adds?  `guides.sh` plans against MAGIC's abstract and the top routes against the PATCHED one, so the two views differ exactly inside the macros (#913).  Measured inert on the N=8 and N=4 H+B arms -- but by **0.555 µm**, about 1.2 met2 track pitches, with hundreds of sub-micron approaches, so it is a fact about today's placement rather than about the mechanism.  It **REFUSES** rather than reporting a zero it did not measure: no notch piece placed, no corridor read, or no layer shared between them each stop it, and liveness is reported PER LAYER because a layer carrying corridors and no notch metal (met3 here) contributes a zero that is arithmetic, not evidence |
