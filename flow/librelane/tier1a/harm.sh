...
set -euo pipefail
N=${1:?usage: harm.sh N [--pins <dir>] [--density PCT] [--halo HX HY] [--margins ROWS SITES]   (after gen.sh N)}; shift || true
pins=()
while [ $# -ge 2 ]; do
    case $1 in
        --pins)    pins+=(--pins "$2"); shift 2 ;;
        --density) pins+=(--density "$2"); shift 2 ;;   # PL_TARGET_DENSITY_PCT for the blocks
        --margins) [ $# -ge 3 ] || { echo "harm.sh: --margins needs ROWS SITES" >&2; exit 1; }
                   pins+=(--margins "$2" "$3"); shift 3 ;;  # the blocks' core margins (rows, sites)
