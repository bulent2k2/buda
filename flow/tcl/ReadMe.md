...
| [`soc.tcl`](soc.tcl) + [`soc_lib.tcl`](soc_lib.tcl) | an **SoC-shaped design** — DEEP and DIVERSE: eleven leaf cell types, ragged depth (an ALU four levels down, a UART two), size a dial. Its own chapter: [`soc.md`](soc.md) |
...
| [`converge.tcl`](converge.tcl) + [`converge_lib.tcl`](converge_lib.tcl) | the **E1/E5 loop driver** ([convergence ladder](../../docs/internal/convergence_ladder.md) item 5): the blind layer-budget policy as scripted rounds against the derived budget (`derive_cell_layer_shares`, or the positional `derive_cell_layer_reserves` under `-primitive reserve`), and — `-arms uniform` — the CONVENTIONAL feedthrough reservation (`set_cell_layer_reserve * TOP uniform F`, F doubling from `-f0` until clean, past `-fmax`, or past the smallest cell's supply), on `soc.tcl`/`tpu.tcl` through the shared hooks (`-reserve`/`-shares`/`-derive`/`-primitive`/`-uniform`/`-plan`/`-derive_plan`/`-noheal`/`-report`), and — `-handdown` — the top's PLAN handed down with the budget (ladder item 6c: `derive_top_plan` → `pin_plan`, the measurement round's globally planned selections, layers and seats re-pinned in every informed round, with `plan` and `fixpoint` columns in the table), and — `-yield` — the reserve derivation's seat policy (ladder item 6d, `derive_cell_layer_reserves yield`: where the corridor leaves a block's own seat no run of consecutive free tracks its bus needs, the block keeps its seat and the top takes the loss, nested templates' inherited corridors included); tables in [convergence_e1.md](../../docs/internal/convergence_e1.md) and [convergence_e5.md](../../docs/internal/convergence_e5.md) |
...
**The chapter moved to [`soc.md`](soc.md)** — it had grown to four fifths of
this file, which made the page about one vehicle rather than about the set.
...
sitting 2, 3 or 4 levels down depending on its subsystem — the two shapes
`tpu.tcl` (a uniform mesh) and `flow/chip` (uniform in depth) do not have.
Clean top-down at every size measured, NQ = 1 through 64.

`soc.md` carries the rest: the face rule read three ways and its mirror, the
`-bottomup` channel that turned out to be one supply-doomed segment, and the
measured tables behind both.

## Flattened `.buda` snapshots

Two fixed-size runs recorded as straight-line `.buda`, for anyone who wants the
design without a `tclsh`:

| file | from | measured here |
|---|---|---|
| [`flow/soc_small.buda`](../soc_small.buda) | `soc.tcl 8` | ~3 s, 323 bundles, 9328 bit-wires |
| [`flow/soc_mid.buda`](../soc_mid.buda) | `soc.tcl 32` | ~25 s, 1235 bundles, 35096 bit-wires |

They are a **flattening**, not a parameterization — one concrete design with
the loops unrolled — so the dial lives in `soc.tcl` and nowhere else. Both are
regenerated by `tools/tcl2buda.py`; each file's header carries its own command.
