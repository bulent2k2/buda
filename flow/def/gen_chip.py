...
def _bus(prefix, i):
    """A bus BIT, written the way a netlist writes one.

    The internal buses are real vectors (`wire [3:0] w;` + `w[0]`), which is
    how buses are actually written and what makes this vehicle exercise the
    bit-select path.  PORTS stay scalar (`A0`..`A3`): BUDA models one pin per
    port NAME, so a vector port is a separate open (interchange item 2's
    remaining half) and writing one here would test nothing that works."""
    return f"{prefix}[{i}]"
