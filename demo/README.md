...
| `feedthrus.buda` | `set_feedthru` in a **hierarchical** flow: a tall relay block the trunk would silently pass over, walked through the whole resolution ladder — baseline pass-through, `(*,layer)` on, `(block,*)` off beating it, `(block,layer)` on beating that — plus a **cell-local** child fed through by its LOCAL name (one rule governs every tile instance), each state shown in a `dump_topologies --conn` transcript, then routed clean end to end. No `visualize`, so `btcl -i` reaches the prompt. Pinned by `test_feedthrus_demo.py`. |
