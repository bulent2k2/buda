...
"""BUDA-1908 — `set_feedthru` on a `teg_mode over` multi-rect block
(teg_multirect_status.md limitation 7).

A feedthru relay and `teg_mode over` are contradictory declarations about the
same thing, the block's own internal routing: `over` says the rects are NOT
internally connected, a feedthru says the block relays the bus through its own
routing.  The engine refuses the combination (add_trunk's relay loop skips an
OVER multi-rect block), and the CLI handler says so at DECLARATION time — once
per command, naming every affected block — while the declaration still takes
effect for every other block it names, THRU multi-rect blocks included (whose
relay IS honoured — see test_feedthru_multirect.py).

Until 2026-08-27 the engine skipped EVERY multi-rect block and this warning
covered all of them; the geometry tests for the THRU half live in
test_feedthru_multirect.py, and the silence assertions below are what keep
this file from re-warning about them.
"""
import contextlib
import io
...
_SETUP = [
    ...
    "add_block L rect 200 0 300 400 rect 200 0 600 100 teg_mode over",
    "add_block T rect 700 300 900 400 rect 700 0 900 100 teg_mode over",
    # A THRU multi-rect block: the relay is honoured, so it is never warned.
    "add_block P rect 1000 0 1100 400 rect 1000 0 1400 100",
]
...
def test_wildcard_warns_once_listing_every_affected_block():
    ...
    # One warning, both OVER blocks named (sorted); neither the single-rect
    # block nor the THRU multi-rect one, whose relays are honoured.
    assert "L, T" in line
    assert "solo" not in line and "P" not in line.split(":")[-1]
    assert s.fp.get_feedthru("solo", 4)
    assert s.fp.get_feedthru("P", 4)
...
def test_no_warning_for_a_thru_multirect_block():
    # The relay is honoured on a THRU block (its rects ARE declared
    # internally connected), so there is no dropped intent to report.
    s = _session(_SETUP)
    out = _do(s, "set_feedthru P * on")
    assert "BUDA-1908" not in out, out
    assert s.fp.get_feedthru("P", 4)
    # ... and the add_block-side twin: a THRU multi-rect block declared
    # under an already-active wildcard stays silent too.
    s2 = _session(["def_layer 4 M4 H TOP 0", "set_feedthru * * on"])
    out2 = _do(s2, "add_block P rect 0 0 100 400 rect 0 0 400 100")
...
def test_wildcard_before_block_declaration_warns_at_add_block():
    ...
    out = _do(s, "add_block L rect 200 0 300 400 rect 200 0 600 100 teg_mode over")
    assert out.count("BUDA-1908") == 1, out
    line = next(l for l in out.splitlines() if "BUDA-1908" in l)
    assert "WARNING" in line and "'L'" in line and "teg_mode over" in line
...
def test_inactive_or_disabled_settings_stay_silent_at_add_block():
    ...
    out2 = _do(s2, "add_block L rect 200 0 300 400 rect 200 0 600 100 teg_mode over")
    assert "BUDA-1908" not in out2, out2
...
def test_both_sites_share_one_memo_no_double_fire():
    ...
    out3 = _do(s, "add_block T rect 700 300 900 400 rect 700 0 900 100 teg_mode over")
