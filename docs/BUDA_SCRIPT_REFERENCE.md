...
| Hier | `set_cell_layer_cap`, `set_cell_layer_share`, `set_cell_layer_reserve` | Per-cell layer band, a fractional per-layer share inside or above it, and the positional reservation — named cell-local tracks the cell leaves free for the top |
