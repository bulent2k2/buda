...
| [Track assignment (NUTS)](script_reference/nuts.md) | 4, 9 | `run_nuts` · `run_nuts_on_layer` · `run_detailed_nuts` · `set_pair_align_heal` · `set_placed_endpoints` · `ripup_reroute` · `negotiate_congestion` · `refine_selection` · `report_wirelength` · `report_layer_demand` · `derive_cell_layer_shares` |
