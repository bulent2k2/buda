...
| [Planner](script_reference/planner.md) | 3, 4c | `set_planner_param` · `run_planner` (+ `hier`, `post_nuts`) · `select_topology` · `select_topologies` · `pin_plan` · `unpin_topology` · `dump_pins` · `set_bus_layers` · `retire_sidecar` |
| [Track assignment (NUTS)](script_reference/nuts.md) | 4, 9 | `run_nuts` · `run_nuts_on_layer` · `run_detailed_nuts` · `set_pair_align_heal` · `set_placed_endpoints` · `ripup_reroute` · `negotiate_congestion` · `refine_selection` · `report_wirelength` · `report_layer_demand` · `derive_cell_layer_shares` · `derive_cell_layer_reserves` |
...
| [Verification & visualisation](script_reference/verify_viz.md) | verify / — | `check_design` · `dump_topologies` · `visualize` · `visualize_topologies` · `emit_guides` · `export_def_blockages` · `emit_pin_def` · `emit_block_size` · `dump_messages` |
...
| 3b | `pin_plan` | ONE bundle's plan handed down from a previous session (ladder item 6c, the lines `derive_top_plan` writes): candidate by uid then type spec, forced layers, abstract seats as slide windows NUTS must honour; held until `run_planner`, audited after `run_nuts` (`[PlanPin] seated S of N`), a template's bundle skipped and said |
...
| 9 | `run_detailed_nuts` (alias `run_dnuts`) | Snap each bus segment's bits to concrete signal-track positions |
...
| — | `report_layer_demand` (alias `report_demand`) | Per instance and layer, the signal tracks over the instance's footprint the REST of the design has placed against the tracks it has — the top plan's demand on a block, the number a `set_cell_layer_share` complement starts from (`buda::query demand` returns the same rows to Tcl) |
| — | `derive_cell_layer_shares [apply] [file <path>] [cells <a,b,...>] [nofloor]` | The complement of the top's demand per cell and layer as `set_cell_layer_share` lines (worst instance, floored — and floored again by the cell's own worst seat, subtree included, since a uniform share below what the block's own bus needs strands it; `nofloor` is the pure complement), with a collision count for the top's tracks inside the slots the thinning keeps; `apply` declares them, `file` writes them for a session that sources them before `run_planner hier` — convergence ladder item 4 |
| — | `derive_cell_layer_reserves [apply] [file <path>] [cells <a,b,...>]` | The positional twin: per cell and layer, the union over the cell's instances of the tracks the top placed over them, in the cell's frame, as `set_cell_layer_reserve` lines — with `seat_hit` (reserved tracks inside the cell's own worst seat) and `own_hit` (its own metal on them now) reported, not refused; same scope rule and doors — convergence ladder item 6 |
| — | `derive_top_plan [file <path>] [cells <a,b,...>]` | The top's PLAN handed down (ladder item 6c): every globally planned bundle's selection (uid + type spec), layers and seats as `pin_plan` lines for the next session to source before `run_planner hier` — so the blocks route under the SAME top the reservation came from; same scope rule as the derivations |
...
| Hier | `set_cell_layer_cap`, `set_cell_layer_share`, `set_cell_layer_reserve`, `set_reserve_steer` | Per-cell layer band, a fractional per-layer share inside or above it, and the positional reservation — named cell-local tracks the cell leaves free for the top — with `set_reserve_steer` (opt-in) making it a corridor the crossing buses are seated on |
...
| [`set_cell_layer_reserve <cell> <layer> <pos>[,<pos>...]\|off`](BDB_REFERENCE.md#set_cell_layer_reserve) | Positional reservation: the cell-local track centres the cell's own routing leaves free for the top — enforced as keepouts on the cell-local solve and the reference DNUTS view where the cell is a `set_bottom_up` template (BUDA-1920 otherwise). |
| [`set_reserve_steer <on\|off>`](BDB_REFERENCE.md#set_reserve_steer) | The reservation's top-side half (ladder item 6b, **default off**): on, every governed instance's reserved tracks are a reserve corridor the crossing buses are seated on by NUTS and land their bits on first in DetailedNUTS, where the corridor can host them.  A lever, not a default: the mesh control's hit rate 0.00 → 1.00 at no cost, the SoC's informed rounds dirtier at NQ ≥ 4.  Byte-identical with nothing reserved |
