...
| Hier | `set_cell_layer_cap`, `set_cell_layer_share`, `set_cell_layer_reserve`, `set_reserve_steer` | Per-cell layer band, a fractional per-layer share inside or above it, and the positional reservation — named cell-local tracks the cell leaves free for the top — with `set_reserve_steer` (default on) making it a corridor the crossing buses are seated on |
...
| [`set_reserve_steer <on\|off>`](BDB_REFERENCE.md#set_reserve_steer) | The reservation's top-side half (ladder item 6b, default on): every governed instance's reserved tracks are a reserve corridor the crossing buses are seated on by NUTS and land their bits on first in DetailedNUTS; `off` is the study control (the block leaves the tracks free, nothing steers the top there — E5's 6–11 % hit rate).  Byte-identical with nothing reserved |
