...
| `G` (Shift+g) | While a trunk is **armed**: drop a **temporary Hanan line** at the cursor on the armed axis (a row for `T`, a column for `Y`) and hover it — the escape for an **empty channel** between blocks, where the bundle grid has no line and the trunk could only snap to the blocks' faces (e.g. the `c_ddd_detour` channel trunks at y=850/550). The line joins the grid (snap target + display) for the session; the placed trunk's `[edit-cmd]` log carries the explicit coordinate, so replay needs no grid. Pair with `W` to restrict each channel trunk's slide to its half of the channel. |
