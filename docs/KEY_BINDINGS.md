...
| `P` (Shift+p) | **Pin the selected segment's span to a subset of busterms**: click the busterm blocks the trunk should reach (each toggles; picked blocks outline red and the resulting span previews live), then `enter` to apply the span (`esc` cancels). Lets two trunks on the same grid line each cover a limited span — the along-axis span shrinks to the picked blocks' extent instead of the full busterm reach. |
