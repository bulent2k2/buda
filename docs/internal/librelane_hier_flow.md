...
| Bus wiring as FIXED pre-routes | DEF `NETS … + FIXED` | NEW, gated on measurement B; planned in [signal_preroutes.md](signal_preroutes.md) (BUDA's own wiring, complete and partial, measurements B′/C/D) |
...
guides left QoR on the table — B passed, and the plan for BUDA writing them
itself, including a pre-route that stops short of a pin with the router
guided the last mile, is [signal_preroutes.md](signal_preroutes.md); `set_bottom_up` on the repeated cell so BUDA's
