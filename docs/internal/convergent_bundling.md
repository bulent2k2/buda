...
`set_max_bundle_bits` is likewise hier-aware (split at `run_hier_bundler` on
TEMPLATE bundles before per-instance expansion, propagating through the
template↔replica linkage).  Tests:
