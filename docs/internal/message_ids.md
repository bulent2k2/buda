...
| BUDA-1503 | WARNING | a restored checkpoint holds a routed design but no routing grid (pre-v29), so re-solving it uses a different physical model |
...
| BUDA-1617 | WARNING | two of a technology LEF's routing layers derive the same layer id, so the stack was numbered by the file's own order rather than by its names |
| BUDA-1618 | WARNING | a technology LEF's routing layers collide on a layer id AND a script `def_layer` fixes one of them where the file's own order cannot put it, so neither numbering applies and the colliding layers are skipped |
...
| BUDA-1713 | WARNING | `emit_pin_def snap` moved pins onto the block-frame tracks because the instance origin is off the track period; the largest shift is reported |
| BUDA-1714 | WARNING | `emit_pin_def on_mismatch reference` took a disputed pin from the reference instance because the cell's instances route it to different places; the jog every other instance is left with is reported |
| BUDA-1715 | WARNING | `emit_pin_def` moved a pin off the track its bit-wire reached the face on because another net's pin already held that metal |
...
| BUDA-1908 | WARNING | `set_feedthru` enables a relay on a `teg_mode over` multi-rect block, which declares its rects NOT internally connected — the two contradict, so the feedthru is refused for that block |
...
| BUDA-1920 | WARNING | a cell holds a positional track reservation (`set_cell_layer_reserve` — its own, or one inherited from a reserved ancestor) but is planned top-down.  A reservation binds the cell-local TEMPLATE solve and the reference DNUTS view, so on a cell that is not `set_bottom_up` it is NOT enforced — the cell's own metal may land on the reserved tracks; `check_design`'s `LAYER_RESERVE` audit reports it.  Mark the cell `set_bottom_up` to enforce |
| BUDA-1921 | WARNING | a reserved cell's 90°-rotated instance class is not governed by the cell's own reservation (`set_cell_layer_reserve`): the positions are stated in the upright template frame and a rotation swaps the axes, so a reserved track has no image on the same layer there.  The class plans through its own clone template; corridors inherited from ancestors still apply, and the derivation folds no rotated instance in |
