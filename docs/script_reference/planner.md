...
| `kPeak` | `0` (**off**) | Routability-aware selection (opt-in; wishlist-planner *"Selection basis"* lever 1). The `kCong` term above is **overflow-only** — zero for any band below capacity — so candidate ranking cannot tell a band others filled to 95% from an empty one until it bursts. `kPeak` adds `kPeak × peak_util`, where `peak_util` is the maximum **existing** fill fraction (`usage/cap`, pre-charge) over the bands the segment would use — and the same term joins the slide-window **band choice** (`best_band_perp`), so the charge itself moves to the emptier band rather than merely pricing the legacy nearest-band pick. A candidate squeezing into an already-loaded corridor now loses to one that detours around it **before** any overflow. Pre-charge deliberately: post-charge utilization was measured and rejected — on an uncongested design it degenerates into an intrinsic "narrow channel" penalty that biases against the column channels the `BITRUNK` datapath trees use (datapath WL regressed across the sweep). Measured at `kPeak 0.1` on the congested corpus: `channel_stress` heals its 3 keepout-open bits at **zero** overlap cost; `rnr/mix` DNUTS unplaced 190→**128** (−33%); trunk-dominated `tc3a_flat` stays clean (0.05–0.1; 0.2 regresses). **Not a safe default (decided 2026-07-11, stays opt-in)**: on `big2`'s plain pipeline the knob strands two wide trunks (56/60 bits) on M4 windows with near-zero real signal-track supply — 0 opens → **116** at every tested value, in BOTH capacity modes (width and `signal_tracks`). The mechanism is structural: `peak_util` prices *relative* existing load, so an almost-supply-free band at utilization 0 looks maximally attractive — the term is blind to *absolute* supply. `negotiate_congestion` + `ripup_reroute` fully heal the damage (big2 → 0 overlaps / 0 opens, same endpoint as baseline), so the knob is safe — and useful — in flows that run the feedback loops, but a default must not depend on them, and the best value is flow-dependent anyway (`mix` optimum 0.1, non-monotonic at 0.2). Prefer enabling it together with `negotiate_congestion`/`ripup_reroute` on congested designs. A supply-aware refinement (deter a band whose absolute signal-track supply in the segment's span cannot host the bundle's bits) would remove the failure mode and reopen the default question — tracked in wishlist-planner. Default `0` skips the term entirely — existing flows are bit-identical. |
...
2. Scores every topology candidate — for each segment independently selects
   ...
   off nearly-full bands *before* they overflow (see `set_planner_param
   kPeak`).
