...
```
buda [-nv | --no-viz | -v | --visualize] [-t | --tag TAG] [-l | --log] [-j | --threads N|max] [--verbose-conn] [--strict-check] [--report-json PATH] [--ipc-verbose] [-h] <script.buda>
```
...
| `-v`, `--visualize` | off | **Open a viewer on the finished design even when the script has no `visualize`** — for running a test vehicle without editing it to add one. After the flow completes it runs `visualize` on the final state, then honors the script's exit code once the window closes. A **no-op if the flow already ends by visualizing**: a trailing `visualize`/`visualize_topologies`, or one immediately before `exit`. A viewer that appears EARLIER in the flow does *not* count — a fresh final view of the finished design still opens. Mutually exclusive with `--no-viz`. The Tcl twin is `btcl -v <flow>.tcl` (see [TCL_FRONT_END.md](TCL_FRONT_END.md)). |
...
| `-j N`, `--threads N\|max` | max/2 | Worker threads for the **parallel pipeline stages**: the planner's candidate scoring, NUTS's per-layer solvers, and the healers' trial sweep. Results are **identical at any thread count** — each stage's parallelism is decision-identical by construction. Min 1; max = the number of logical CPUs available to this process (affinity- **and** CFS-cpu-quota-aware — on hybrid CPUs that is exactly performance-cores × SMT + efficiency-cores); an out-of-range request is clamped with a warning, never an error. **`-j max`** uses the whole machine maximum — the explicit opt-out of the default. Default (flag absent): **half the machine maximum**. Precedence: an explicit `--threads` (a number **or** `max`) overrides the three per-engine env vars (`BUDA_PLAN_THREADS` / `BUDA_NUTS_THREADS` / `BUDA_SWEEP_THREADS` — explicit semantics, which also bypass the engines' small-work gates, as a hand-set per-engine var always did); the flag-absent default sets only the **`BUDA_THREADS`** governor, a **ceiling** the engines' auto paths honor with their small-work gates intact — so ordinary runs never spawn pools for tiny candidate/layer sets, and per-engine env overrides keep working without the flag. The Tcl launcher **`btcl` honors the same `-j`/`-j max`** with the same defaults (see [TCL_FRONT_END.md](TCL_FRONT_END.md)). |
