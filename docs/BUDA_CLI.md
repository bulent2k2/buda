...
```
buda [-nv | --no-viz] [-t | --tag TAG] [-l | --log] [-j | --threads N] [--verbose-conn] [--ipc-verbose] [-h] <script.buda>
```
...
| `-j N`, `--threads N` | max/2 | Worker threads for the **parallel pipeline stages**: the planner's candidate scoring, NUTS's per-layer solvers, and the healers' trial sweep (their `BUDA_PLAN_THREADS` / `BUDA_NUTS_THREADS` / `BUDA_SWEEP_THREADS` env knobs, all set at once). Results are **identical at any thread count** — each stage's parallelism is decision-identical by construction. Min 1; max = the number of logical CPUs available to this process (cgroup/affinity-aware — on hybrid CPUs that is exactly performance-cores × SMT + efficiency-cores); an out-of-range request is clamped with a warning, never an error. Default (flag absent): **half the machine maximum**. Precedence: an explicit `--threads` overrides the three env vars; the default only fills the ones you have not set, so per-engine env overrides keep working. |
